...
class TCPThread(QThread):
    ...
    def __init__(self, psd_window=mlab.window_hanning ) :
        ...
        self.dcrf = uio.DataCaptureRF(uio.DEV_RF_CAPTURE)
    ...
    def run(self):
        ...
        while self.isRunning:
            # read 2048 words (32 bits) = 8192 bytes
            self.dcrf.Read()
            self.send_signalRx()
            time.sleep(0.01)
    ...
    def send_signalRx(self):
        # iq contains iq interleaved samples. I0 Q0 I1 Q1 ... I2047 Q2047
        # iq length should be 16384 bytes
        countOTR = 0
        iq = self.dcrf.c_array2048
        #ii_int = list(range(0,2048))
        qq_int = list(range(0,2048)) # it should always be 0
        """
        ...
        if(countOTR>0):
            a = 5
        """
        #iq_int_np = np.array(iq) + 1j*np.array(qq_int)
        ...
        power, _ = mlab.psd(iq, NFFT=2048, window=self.PSDwindow, Fs=64e6, scale_by_freq=False)
